...
- 不强求 LLM：缺失时敏感检测退化为关键词+正则，议价检测退化为关键词聚类，封面标题退化为首句字幕，流水线不中断。高风险画面检测在无视觉 LLM 时退化为弱启发式（建议配置多模态模型）。
...
10. **LLM 缺失时自动降级**：敏感检测退化为关键词+正则，议价检测退化为关键词聚类，封面标题退化为首句字幕。不要让缺失的 LLM 阻断整条流水线。
