...
- **智能体生成标题改为「封面标题 + 视频号短标题」分开**：封面标题 `TITLE`（≤20 字，成片封面用，不受 16 字符限制）；视频号短标题 `SHORT_TITLE`（**≤16 字符，视频号后台硬限制，超出无法保存草稿**）。prompt 输出格式新增 `SHORT_TITLE:` 行，sync 命令改用 `--title "<SHORT_TITLE>"`。
