...
- **Mac 上成片画质变糊（auto 编码命中 videotoolbox）** → **必须更新到 v2026.09.02**（之前版本 videotoolbox 的 `-q:v` 质量刻度写反，错用了 crf 值 19/100，画质被压成渣；Windows/QSV 那条不受影响，所以只有 Mac 同事中招）。
...
### 要不要更新
- **Mac 上成片画质变糊** → **必须更新**（auto 编码会命中 `h264_videotoolbox`，旧版本质量刻度方向写反，渣画质；Windows/QSV 不受影响）。

### Fixed
- **⚠️ 修复 macOS 成片画质变糊（videotoolbox 质量刻度写反）**：所有硬件编码器复用了 `final_crf`（默认 19）当质量值，但 `h264_videotoolbox` 的 `-q:v` 是 **1~100「质量」刻度、越大越好**（q 55~65 优秀、45~50 良好），旧代码把 19 直接传进去 → 19/100 渣画质 → 糊。现已单独翻转刻度：`q = max(60, min(95, 118 - round(crf*2)))`，crf19 → **q80**、crf23 → q72（优秀画质），并新增可覆盖配置项 `final_vt_quality`（`src/config.py` / 环境变量 `AVEditor_FINAL_VT_QUALITY`）。`src/render.py` 的 `_video_encode_args` 与此映射已改；`src/config.py` 注释同步。
