...
- **AI 分析**：OpenAI 兼容大模型 API（敏感语义 / 议价分析 / 视觉分类）；缺失自动降级为关键词/启发式，流水线不中断。**封面标题经智能体使用时由智能体自带 LLM 生成并注入，无需外部 Key**；纯命令行无智能体时才需 `AVEditor_LLM_*` 或手动 `--cover-title`，否则留空。
...
| `AVEditor_LLM_API_KEY` / `BASE_URL` / `MODEL` | LLM 连接（敏感语义/议价质量优化；纯命令行封面标题依赖） | 空（纯命令行封面标题留空；经智能体使用时由智能体生成）|
